...
void Monster::init(cWorld* world) {
    m_world = world;
    m_mesh = new cMesh();
...
bool Monster::takeDamage(int damage) {
    ...
    if (m_health <= 0) {
        m_health = 0;
        deactivate();
        return true; // Monster died on 10th hit!
    }
...
void Monster::update(double dt) {
    ...
    // Smooth time step for 3D sweeping & vertical movement (additional 30% slower frequency)
    m_pulseTime += dt * 0.27;
    ...
    // Smooth sinusoidal horizontal sweeping from Left (-4.0m) to Right (+4.0m)
    double wideAmplitude = 4.0;
    double newX = wideAmplitude * std::sin(m_pulseTime);

    // Dynamic 3D vertical up-and-down motion (Bobbing height between 0.3m and 3.5m high in the air)
    double newZ = 1.90 + 1.60 * std::cos(m_pulseTime * 1.8);
    ...
    if (m_mesh) {
        ...
        double dmgRatio = 1.0 - healthRatio; // 0.0 (fresh red) -> 0.9 (pale gray)
